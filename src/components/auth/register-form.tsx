...
import { inviteJoinPath } from "@/lib/auth/google";
...
export function RegisterForm({
  ...
  const callbackUrl = useMemo(
    () => (inviteToken ? inviteJoinPath(inviteToken) : "/dashboard"),
    [inviteToken],
  );

  const loginHref = inviteToken
    ? `/login?email=${encodeURIComponent(prefilledEmail || email)}&callbackUrl=${encodeURIComponent(callbackUrl)}`
    : "/login";
      ...
      <div className="w-full max-w-md space-y-8 animate-fade-in">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-2 text-sm">
            {inviteToken
              ? "Use Google or email — you’ll join the project right after."
